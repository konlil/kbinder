// Bindle.cpp : Defines the class behaviors for the application.
//
...
BEGIN_MESSAGE_MAP(CBindleApp, CWinApp)
	ON_COMMAND(ID_HELP, &CWinApp::OnHelp)
END_MESSAGE_MAP()


// CBindleApp construction

CBindleApp::CBindleApp()
{
...
// The one and only CBindleApp object

CBindleApp theApp;


// CBindleApp initialization

BOOL CBindleApp::InitInstance()
{
	...
	if( __argc > 1 )
	{
		...
		{
			if( stricmp(__argv[i], "-b") == 0 )
			{
				binding = true;
				i++;
				exe_to_bind.push_back(string(__argv[i]));
			}
			else if( stricmp(__argv[i], "-o") == 0 )
			{
				i++;
				target_name = string(__argv[i]);
			}
			else if( stricmp(__argv[i], "-s") == 0 )
			{
				i++;
				my_org_size = atoi(__argv[i]);
			}
		}
	}
